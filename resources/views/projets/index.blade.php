            ...
            <tbody>
                @foreach ($projets as $projet)
                    <tr class="bg-primary">
                        ...
                        <td>{{ $projet->date_cloture }}</td>
                        <td>
                            <span class="badge bg-{{ $projet->statut_color }}">
                                {{ $projet->statut_text ?? 'N/A' }}
                            </span>
                        </td>
                        <td>{{ $projet->description }}</td>
                        <td>{{ $projet->user->name ?? 'N/A' }}</td>
                        <td>
                            <div class="dropdown">
                                <button class="btn btn-light btn-sm dropdown-toggle" type="button"
                                    id="actionMenu{{ $projet->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                    &#x22EE;
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="actionMenu{{ $projet->id }}">
                                    <li><a class="dropdown-item"
                                            href="{{ route('projets.edit', $projet->id) }}">Modifier</a></li>
                                    <li>
                            ...
                            <form id="deleteForm{{ $projet->id }}" action="{{ route('projets.destroy', $projet->id) }}"
